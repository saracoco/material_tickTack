...
all_res = lapply(res_names, function(r) {
  ...
  sub_res = lapply(sub_dirs, function(sub_i) {
    ...
    if (file.exists(paste0(fp, '/sim.rds'))&
        ...
        file.exists(paste0(fp, '/res_gritic.rds'))) {
      ...
      res_gr = readRDS(paste0(fp, '/res_gritic.rds'))
      selection_gr <- sim$cn %>% 
        mutate(row_id = row_number()) %>% 
        filter(paste0(chr,"-",startpos,"-",endpos)%in%res_gr$Segment_ID)
      taus_clust_gr = sim$taus_clust[selection_gr$row_id]
      ri_gr = catsim::rand_index(cl_gr, taus_clust_gr)
      removed_segments_gritic = length(sim$taus_clust)-length(cl_gr)
      ...
      # if (unique(sim$taus_clust)!=1){
        dplyr::tibble(n_clocks, n_events, purity,coverage, n_mutations, i.iter = sub_i, ri_at, ri_mt, ri_tt, ri_tth, ri_gr, removed_segments_gritic)  
      # } else {
...
saveRDS(all_res, "results_summarised_gritic/clustering_results_test.RDS")
