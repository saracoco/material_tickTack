...
info_segments <- readRDS("./data/info_complex_segments_5e+06.rds")
info_segments

fittable_samples <- readRDS(paste0("./data/fittable_paths.rds")) #fittable_samples_5Mb_smoothed_15mm_0.4pi
...
df %>%
  filter(karyotype != "1:1" & karyotype != "1:0" & karyotype != "0:0") %>% 
  count(karyotype) %>%
  ...
  labs(title = "Karyotype Distribution (Filtered by Percentage (> 0.01), Excluding 1:1,1:0,0:0)",
       y = "Karyotype",
       x = "Count") +
...
df %>%
  ...
  theme(panel.background = element_rect(fill = "white", color = NA),
        plot.background = element_rect(fill = "white", color = NA)) +
  labs(title = paste0("Karyotype Distribution Across Tumour Types (Filtered, Excluding 1:1,1:0,0:0, percentage > ", percentage_threshold, " )"),
       x = "Karyotype",
       y = "Count") +
  scale_fill_viridis_d() +
  guides(fill = guide_legend(title = "Karyotype"))  
