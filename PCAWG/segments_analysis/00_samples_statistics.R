...
for (i in c(5e6)) {
  info_segments <- future_lapply(1:(length(sample_files)), function(idx) {
    ...
    n_info_per_segment_complex <- lapply(1:(length(cna_complex)), function(segment_idx) {
    
      segment <- cna_complex[segment_idx, ]
      chr <- segment$chr                                            
      segment_id <- paste(chr, segment$from, segment$to, sep = "_")
      k <- segment$k
      ...
      segment_length <- segment$to - segment$from
      
      data_frame_single = tryCatch(
        dplyr::tibble(sample = unique(cnaqc_x$mutations$sample), segment_mutations=nrow(segment_mutations), segment_id=segment_id, karyotype=k, segment_length=segment_length, type = tryCatch(
  ...
  #   )
  #   
    return(n_info_per_segment_complex)

  }) %>% bind_rows()
  
  saveRDS(info_segments, paste0("./data/info_complex_segments_", i, ".rds"))
}
